...
function findSelected(rungs: Rung[], selection: Selection | null): Block | null {
  ...
  if (selection.kind === 'parallel') return rung.parallel?.id === selection.blockId ? rung.parallel : null;
...
export function SimurelayStyleLadderScreenV10() {
  ...
  function invertSelected() {
    ...
    setRungs((current) => {
      ...
      next.forEach((rung) => {
        ...
        if (rung.parallel?.id === selection.blockId) rung.parallel.type = rung.parallel.type === 'NO' ? 'NC' : 'NO';
      });
      return next;
    });
  }
  ...
  function duplicateSelected() {
    ...
    setRungs((current) => {
      ...
      if (selection.kind === 'parallel' && rung.parallel) {
        const copy = { ...rung.parallel, id: uid('copy') };
        rung.series.push(copy);
        setSelection({ rungId: rung.id, blockId: copy.id, kind: 'series' });
      }
      return next;
    });
  ...
  function removeSelected() {
    ...
    setRungs((current) => {
      ...
      next.forEach((rung) => {
        ...
        if (rung.parallel?.id === selection.blockId) rung.parallel = undefined;
      });
      return next;
    });
    setSelection(null);
  ...
  function createParallelFromSelected() {
    ...
    setRungs((current) => {
      ...
      const rung = next.find((item) => item.id === selection.rungId);
      if (!rung) return current;
      rung.parallel = { ...block, id: uid('parallel') };
      setSelection({ rungId: rung.id, blockId: rung.parallel.id, kind: 'parallel' });
      return next;
    });
  }
      ...
      <View style={styles.root}>
        <View style={styles.header}>
          ...
          <View style={styles.titleBox}>
            ...
            <Text style={styles.subtitle}>{selectedBlock ? `${selectedBlock.address} selecionado` : activeTool ? `${activeTool} na mão · toque no +` : 'workspace ladder'}</Text>
          </View>
        ...
        <View style={styles.workspace}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.canvasX}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.canvasY, { minHeight: canvasHeight }]}>
              <Pressable style={[styles.canvas, { height: canvasHeight }]} onPress={() => setSelection(null)}>
                <Grid />
          ...
          {selectedBlock && !simulating ? <FloatingToolbar block={selectedBlock} onInvert={invertSelected} onDuplicate={duplicateSelected} onParallel={createParallelFromSelected} onDelete={removeSelected} /> : null}
...
function RungView({ rung, index, top, active, activeBlocks, showZones, hotZone, selection, onDrop, onSelect }: { rung: Rung; index: number; top: number; active: boolean; activeBlocks: Record<string, boolean>; showZones: boolean; hotZone: Zone | null; selection: Selection | null; onDrop: (zone: Zone) => void; onSelect: (selection: Selection) => void }) {
  ...
  return <View style={[styles.rung, { top }]}>
    ...
    <View style={[styles.rungRow, { gap: spacing }]}>
      ...
      {rung.series.map((block, blockIndex) => <View key={block.id} style={[styles.inline, { gap: spacing }]}><BlockView block={block} active={Boolean(activeBlocks[block.id])} selected={selection?.blockId === block.id} onPress={() => onSelect({ rungId: rung.id, blockId: block.id, kind: 'series' })} /><ZoneButton visible={showZones} hot={isHot('series', blockIndex + 1)} onPress={() => onDrop({ rungId: rung.id, kind: 'series', index: blockIndex + 1 })} /></View>)}
      <View style={[styles.fillWire, active && styles.wireLive]} />
      <BlockView block={rung.coil} active={Boolean(activeBlocks[rung.coil.id])} selected={selection?.blockId === rung.coil.id} onPress={() => onSelect({ rungId: rung.id, blockId: rung.coil.id, kind: 'coil' })} />
    </View>
    <View style={styles.parallelRow}>
      ...
      {rung.parallel ? <BlockView block={rung.parallel} active={Boolean(activeBlocks[rung.parallel.id])} selected={selection?.blockId === rung.parallel.id} onPress={() => onSelect({ rungId: rung.id, blockId: rung.parallel?.id ?? '', kind: 'parallel' })} /> : null}
    </View>
  </View>;
}

function BlockView({ block, active, selected, onPress }: { block: Block; active: boolean; selected: boolean; onPress: () => void }) {
  const symbol = block.type === 'COIL' ? '( )' : block.type === 'NO' ? '| |' : '|/|';
  return <Pressable style={[styles.block, selected && styles.blockSelected]} onPress={onPress}><Text style={[styles.blockLabel, active && styles.liveText]}>{block.address}</Text><Text style={[styles.symbol, active && styles.liveText]}>{symbol}</Text></Pressable>;
}

function FloatingToolbar({ block, onInvert, onDuplicate, onParallel, onDelete }: { block: Block; onInvert: () => void; onDuplicate: () => void; onParallel: () => void; onDelete: () => void }) {
  const contact = block.type !== 'COIL';
...
const styles = StyleSheet.create({
  ...
  block: { minWidth: 72, alignItems: 'center', justifyContent: 'center', borderRadius: 14, paddingHorizontal: 4, paddingVertical: 4 }, blockSelected: { backgroundColor: 'rgba(50,214,90,0.14)', borderWidth: 1, borderColor: '#32d65a', shadowColor: '#32d65a', shadowOpacity: 0.7, shadowRadius: 10, elevation: 5 }, blockLabel: { color: '#29bc4c', fontSize: 18, fontWeight: '900', marginBottom: 8 }, liveText: { color: '#58f073' }, symbol: { color: '#f2f2f2', fontSize: 45, lineHeight: 50, fontWeight: '300' }, zone: { width: 31, height: 31, borderRadius: 20, borderWidth: 1, borderColor: '#32d65a', backgroundColor: 'rgba(50,214,90,0.12)', alignItems: 'center', justifyContent: 'center' }, zoneHot: { transform: [{ scale: 1.25 }], backgroundColor: 'rgba(50,214,90,0.32)', shadowColor: '#32d65a', shadowOpacity: 0.85, shadowRadius: 14, elevation: 8 }, zoneText: { color: '#32d65a', fontSize: 22, fontWeight: '900' }, zoneTextHot: { color: '#fff' }, emptyZone: { width: 0, height: 31 },
  toolbar: { position: 'absolute', left: 24, bottom: 58, minHeight: 46, maxWidth: '92%', borderRadius: 999, backgroundColor: 'rgba(20,20,20,0.96)', borderWidth: 1, borderColor: '#303030', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, zIndex: 6 }, toolbarLabel: { color: '#32d65a', fontWeight: '900', marginRight: 4 }, toolbarButton: { minHeight: 32, paddingHorizontal: 10, borderRadius: 999, backgroundColor: '#232323', justifyContent: 'center' }, toolbarText: { color: '#f5f5f5', fontSize: 11, fontWeight: '900' }, toolbarDanger: { minHeight: 32, paddingHorizontal: 10, borderRadius: 999, backgroundColor: '#3a1515', justifyContent: 'center' }, toolbarDangerText: { color: '#ff6b6b', fontSize: 11, fontWeight: '900' },
