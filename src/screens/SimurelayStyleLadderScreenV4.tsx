...
function closed(block: Block, state: PlcState) {
...
function evaluate(rungs: Rung[], state: PlcState) {
  ...
  rungs.forEach((rung) => {
    const seriesPower = rung.series.every((block) => {
      const power = closed(block, next);
      activeBlocks[block.id] = power;
      return power;
    });
    const parallelPower = rung.parallel ? closed(rung.parallel, next) : false;
...
function nextBlock(type: BlockType, rungs: Rung[]): Block {
...
export function SimurelayStyleLadderScreenV4() {
  ...
  function addBlock(zone: Zone) {
    ...
    setTimeout(() => setHotZone(null), 260);
    setActiveTool(null);
  }
      ...
      <View style={styles.root}>
        <View style={styles.header}>
          ...
          <View style={styles.titleBox}>
            ...
            <Text style={styles.subtitle}>{activeTool ? `${activeTool} na mão` : 'workspace ladder'}</Text>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.canvasX}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.canvasY}>
              <View style={styles.canvas}>
                ...
                {!simulating ? <Pressable style={styles.addRung}><Text style={styles.addRungText}>+ Rung</Text></Pressable> : null}
              </View>
            </ScrollView>
          </ScrollView>
...
function Grid() {
  return <View style={styles.grid}>{Array.from({ length: 260 }).map((_, index) => <View key={index} style={styles.gridCell} />)}</View>;
}
...
function RungView({ rung, index, active, activeBlocks, showZones, hotZone, onDrop }: { rung: Rung; index: number; active: boolean; activeBlocks: Record<string, boolean>; showZones: boolean; hotZone: Zone | null; onDrop: (zone: Zone) => void }) {
  const top = 205 + (index - 1) * 310;
  ...
  return <View style={[styles.rung, { top }]}>
    ...
    <View style={styles.rungRow}>
      ...
      {rung.series.map((block, blockIndex) => <View key={block.id} style={styles.inline}><BlockView block={block} active={Boolean(activeBlocks[block.id])} /><ZoneButton visible={showZones} hot={isHot('series', blockIndex + 1)} onPress={() => onDrop({ rungId: rung.id, kind: 'series', index: blockIndex + 1 })} /></View>)}
...
function GhostPreview({ type }: { type: BlockType }) {
  ...
  return <View style={styles.ghost}><Text style={styles.ghostLabel}>GHOST</Text><Text style={styles.ghostSymbol}>{symbol}</Text><Text style={styles.ghostHint}>toque no + que acende</Text></View>;
}
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#101010' }, root: { flex: 1, backgroundColor: '#101010' }, header: { height: 84, paddingHorizontal: 18, backgroundColor: '#101010', flexDirection: 'row', alignItems: 'center', gap: 14 }, menu: { color: '#f5f5f5', fontSize: 30, fontWeight: '900' }, titleBox: { flex: 1 }, title: { color: '#ffffff', fontSize: 21, fontWeight: '900' }, subtitle: { color: '#bdbdbd', marginTop: 2, fontSize: 14 }, simButton: { flexDirection: 'row', alignItems: 'center', gap: 7 }, simIcon: { color: '#2ecc58', fontSize: 23, fontWeight: '900' }, simText: { color: '#2ecc58', fontWeight: '900' }, more: { color: '#f5f5f5', fontSize: 28, fontWeight: '900' },
  ...
  workspace: { flex: 1, backgroundColor: '#121212', overflow: 'hidden' }, canvasX: { minWidth: 790, flexGrow: 1 }, canvasY: { minHeight: 1020, flexGrow: 1 }, canvas: { width: 820, height: 1040, backgroundColor: '#121212', position: 'relative' }, grid: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, flexDirection: 'row', flexWrap: 'wrap' }, gridCell: { width: 41, height: 41, borderRightWidth: 1, borderBottomWidth: 1, borderColor: '#242424' }, leftRail: { position: 'absolute', left: 58, top: 90, bottom: 120, width: 3, backgroundColor: '#eeeeee' }, rightRail: { position: 'absolute', right: 58, top: 90, bottom: 120, width: 3, backgroundColor: '#eeeeee' },
  rung: { position: 'absolute', left: 58, right: 58, height: 260 }, rungIndex: { position: 'absolute', left: -34, top: 56, color: '#888', fontSize: 16 }, rungWire: { position: 'absolute', left: 0, right: 0, top: 68, height: 3, backgroundColor: '#eeeeee' }, wireLive: { backgroundColor: '#44d761' }, energyPulse: { position: 'absolute', left: 185, top: 61, width: 15, height: 15, borderRadius: 8, backgroundColor: '#44d761' }, rungRow: { position: 'absolute', left: 96, right: 92, top: 18, height: 105, flexDirection: 'row', alignItems: 'center', gap: 28 }, inline: { flexDirection: 'row', alignItems: 'center', gap: 28 }, fillWire: { flex: 1, minWidth: 24, height: 3, backgroundColor: '#eeeeee' }, parallelRow: { position: 'absolute', left: 250, right: 175, top: 118, height: 78, borderLeftWidth: 3, borderBottomWidth: 3, borderRightWidth: 3, borderColor: '#eeeeee', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 30 },
  block: { minWidth: 72, alignItems: 'center', justifyContent: 'center' }, blockLabel: { color: '#29bc4c', fontSize: 18, fontWeight: '900', marginBottom: 8 }, liveText: { color: '#58f073' }, symbol: { color: '#f2f2f2', fontSize: 45, lineHeight: 50, fontWeight: '300' }, zone: { width: 31, height: 31, borderRadius: 20, borderWidth: 1, borderColor: '#32d65a', backgroundColor: 'rgba(50,214,90,0.12)', alignItems: 'center', justifyContent: 'center' }, zoneHot: { transform: [{ scale: 1.25 }], backgroundColor: 'rgba(50,214,90,0.32)', shadowColor: '#32d65a', shadowOpacity: 0.85, shadowRadius: 14, elevation: 8 }, zoneText: { color: '#32d65a', fontSize: 22, fontWeight: '900' }, zoneTextHot: { color: '#ffffff' }, emptyZone: { width: 0, height: 31 },
  ghost: { position: 'absolute', right: 118, bottom: 150, width: 122, height: 112, borderRadius: 18, borderWidth: 1, borderColor: '#32d65a', backgroundColor: 'rgba(50,214,90,0.18)', alignItems: 'center', justifyContent: 'center', shadowColor: '#32d65a', shadowOpacity: 0.8, shadowRadius: 16, elevation: 9 }, ghostLabel: { color: '#baf7c6', fontSize: 10, fontWeight: '900' }, ghostSymbol: { color: '#ffffff', fontSize: 42, marginVertical: 4 }, ghostHint: { color: '#baf7c6', fontSize: 10, fontWeight: '700' },
  drawer: { position: 'absolute', left: 0, top: 0, bottom: 0, width: 350, backgroundColor: '#151515', borderRightWidth: 1, borderColor: '#303030', zIndex: 5 }, drawerHeader: { height: 58, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, drawerTitle: { color: '#eeeeee', fontWeight: '900', letterSpacing: 1 }, drawerClose: { color: '#ffffff', fontSize: 32 }, drawerItems: { padding: 18, flexDirection: 'row', flexWrap: 'wrap', gap: 14 }, drawerHint: { color: '#9d9d9d', paddingHorizontal: 20, fontSize: 13 }, toolCard: { width: 96, height: 92, backgroundColor: '#1f1f1f', borderWidth: 1, borderColor: '#333', alignItems: 'center', justifyContent: 'center' }, toolSymbol: { color: '#f4f4f4', fontSize: 33 }, toolLabel: { color: '#dddddd', marginTop: 8, fontSize: 12 },
  fab: { position: 'absolute', right: 30, bottom: 58, width: 76, height: 76, borderRadius: 38, backgroundColor: '#28a745', alignItems: 'center', justifyContent: 'center', elevation: 10 }, fabText: { color: '#fff', fontSize: 44, lineHeight: 50 }, ioHandle: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(17,17,17,0.92)' }, ioHandleText: { color: '#ffffff', fontSize: 28, fontWeight: '900' }, addRung: { position: 'absolute', left: 165, right: 165, top: 840, height: 48, borderRadius: 999, borderWidth: 1, borderStyle: 'dashed', borderColor: '#2e8b47', alignItems: 'center', justifyContent: 'center' }, addRungText: { color: '#37bf4b', fontWeight: '900' },
  ioSheet: { backgroundColor: '#171717', borderTopLeftRadius: 26, borderTopRightRadius: 26, borderWidth: 1, borderColor: '#303030', paddingBottom: 18, paddingHorizontal: 24 }, sheetHandle: { alignSelf: 'center', width: 72, height: 8, borderRadius: 999, backgroundColor: '#707070', marginTop: 10, marginBottom: 18 }, ioContent: { flexDirection: 'row', gap: 30, paddingBottom: 16 }, ioColumn: { flex: 1 }, ioTitle: { color: '#ffffff', fontSize: 16, fontWeight: '900', marginBottom: 14 }, ioRow: { height: 42, flexDirection: 'row', alignItems: 'center', gap: 16 }, ioLabel: { color: '#f2f2f2', fontSize: 19, fontWeight: '700', width: 58 }, dot: { width: 15, height: 15, borderRadius: 8, backgroundColor: '#666' }, dotOn: { backgroundColor: '#37bf4b' },
});
